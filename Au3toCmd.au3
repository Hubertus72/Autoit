...
; SourceCode:       http://www.autoitscript.com/forum/index.php?showtopic=201562  Version: 2022.05.12
...
Func _IconPath()
	Local $aTemp = StringRegExp($sSourceData, "(?m)\n#AutoIt3Wrapper_Icon=(.*)", 1)
	If IsArray($aTemp) And FileExists($aTemp[0]) Then
		$sIconPath = $aTemp[0]
...
Func _Targetpath()
	Local $sA3Dir, $sA3Ver, $hTargetpath, $t, $x64 = 0, $beta = 0
	$sTargetpath = $sDrive & $sDir & $sFileName & ".cmd"
	FileDelete($sTargetpath)
	$beta = StringRegExp($sSourceData, "(?m)\n#AutoIt3Wrapper_Version=[bByY1]", 0)
	...
	$x64 = StringRegExp($sSourceData, "(?m)\n#AutoIt3Wrapper_UseX64=[yY1]", 0)
	...
	FileWriteLine($hTargetpath, _
			 ...
			 & @CRLF & '  del ~~' _
			 & @CRLF & ')' _
			 & @CRLF & 'call :ts %0 "%r%a3x\%n%.a3x"' _
			 & @CRLF & 'if %t1% geq %t2% (' _
			 & @CRLF & 'mkdir %r%a3x\' _
			 & @CRLF & 'certutil -decode -f %0 "%r%a3x\%n%.a3x"' _
			 & @CRLF & ')' _
			 & @CRLF & 'set wdir=%~dp0' _
			 & @CRLF & 'if not "%wdir: =%"=="%wdir%" set wdir=%~sdp0' _
			 & @CRLF & 'wmic process call create ''"%r%exe\%ver%\AutoIt3%x64%.exe" "%r%a3x\%n%.a3x" "%*"'' ,%wdir% ' _
			 & @CRLF & 'rem timeout /T 999' _
			 & @CRLF & 'exit' _
			 & @CRLF & ':ts t1 t2' _
			 & @CRLF & 'set t1=%~t1' _
			 & @CRLF & 'set t2=%~t2' _
